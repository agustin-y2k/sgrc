...
- **Flujo:**
  ...
  2. Elige **qué clase de equipo es**: una computadora (notebook, tablet, PC de escritorio) o cualquier otra cosa. Es lo primero, porque decide el resto del formulario.
  3. Carga el equipo con **qué es** (tipo, texto libre con sugerencias de los ya cargados) y **cómo lo llaman** (nombre, obligatorio), y su **número de serie** si lo tiene.
  4. Si es una computadora, completa además **los datos de la máquina**: CPU, RAM, sistema operativo, software instalado y si está freezada. Son los mismos campos que los de una computadora de carro.
  5. Decide si **se puede reservar con anticipación**. Por defecto no.
  6. Desde ese momento el equipo se entrega, se recibe y se reclama en las mismas pantallas que las computadoras. Si es una computadora, además se le anotan **licencias** (RF-03.13) y **cuentas de acceso** (RF-03.22) desde su propia ficha.
  7. Puede **editarlo** después —corregir el nombre, cambiar si se reserva, corregir la clase— o **darlo de baja** si se rompió o se perdió.
- **Reglas que no son obvias:**
  - No tienen identificador, y el número de serie es **opcional**: "PC 3" no significa nada para un cargador, y un cargador puede no traer serie de fábrica —un proyector sí, y es de lo que más se extravía—. El **nombre** es lo único que los distingue, y es **único** entre ellos sin distinguir mayúsculas — dos filas llamadas "Cargador" serían indistinguibles justo donde hay que elegir cuál se está prestando.
  - **La clase se pregunta, no se deduce del tipo.** Con el tipo libre, "Note book", "Notebook HP" y "Ultrabook" son la misma cosa para quien las escribe y tres cadenas distintas para cualquier regla que las quiera reconocer: adivinar deja equipos sin ficha sin que nadie entienda por qué. Y se llama *computadora* y no *notebook* porque lo que agrupa a una notebook, una tablet y una PC de escritorio que no vuelve al carro es tener ficha técnica y una cuenta con la que se entra, no el formato.
  - **Desmarcar la clase no borra la ficha**: deja de mostrarse y vuelve intacta si se lo vuelve a marcar. Lo habitual es que se esté corrigiendo un error de carga, y perder lo escrito por una casilla mal tildada es peor que guardar un dato que no se muestra. Por lo mismo, un equipo que ya tiene **cuentas anotadas** conserva el acceso a ellas aunque no esté marcado como computadora.
  - En la lista, **las computadoras van separadas del resto**: se administran distinto, y mezcladas en una sola tira no se distingue cuál es cuál. Los títulos aparecen solo cuando hay de las dos clases.
...
- **Reglas que no son obvias:**
  ...
  - **Solo se listan unas pocas clases**, no la agenda entera: el listado completo está a un clic, y una agenda larga acá abajo empuja los atajos fuera de la vista. Pero el **contador** se calcula sobre el día completo: la consulta pide el máximo por página, porque paginando de a cincuenta un día a full son noventa y cinco reservas y el número decía "6 clases hoy" cuando había once.
...
- **Qué muestra la pantalla de inicio:**
  1. **La tira de estado**: cinco números de alto fijo, arriba de todo. Con cuántos equipos se cuenta *acá ahora* —el total sin los dados de baja, menos lo que está afuera, y cuántos de los que están acá no se pueden entregar por estar fuera de circulación—, cuántos están afuera, cuántas clases hay hoy, cuántas cuentas esperan aprobación y cuántos avisos hay sin leer.
  2. **Para entregar hoy**: las clases del día en orden cronológico, con las que están en curso arriba y marcadas. Cada una dice cuántas de sus computadoras salieron y trae el botón para entregar el resto; las que todavía no empezaron también se pueden entregar antes de hora. **Entregar sin reserva** vive en la cabecera de esta tarjeta.
  3. **Afuera del laboratorio**: todo lo que está prestado —venga de una reserva o de un préstamo suelto— agrupado por quién lo tiene, con el botón para marcar que volvió.
  4. **Lo que viene**: lo reservado para los próximos días.
- **Reglas que no son obvias:**
  ...
  - **"Estar acá" no es "poder entregarse"**: una computadora en mantenimiento está en el laboratorio y no se le da a nadie. Por eso el conteo de presencia y el de circulación se dicen por separado en vez de mezclarse en un total que después nadie sabe leer.
  - *Entregada* **sale de la reserva, no del equipo**: es la reserva que tiene su propio préstamo abierto. Acá decía lo contrario —"la custodia es del equipo"— y de esa frase salió un error que estuvo a la vista sin que nadie lo viera, porque con dos o tres reservas de prueba no se nota: la misma computadora la reservan varias clases a lo largo del día, así que mirando el equipo **una entrega a las once daba por entregado el día entero**. Cargado un día real, nueve de once clases decían tener máquinas afuera que nadie había sacado, y el botón ofrecía entregar dos cuando faltaban diez.
  - Lo que sí es del equipo es **estar afuera**, y eso tiene su propia consecuencia: una máquina reservada para las 13:30 que salió con la clase de las 11:00 no se puede entregar todavía. Se cuenta aparte —"10 todavía con otra clase"— en vez de ofrecerla y que la entrega falle contra el servidor.
  - **El detalle equipo por equipo, sólo en la clase en curso.** Para una clase de la tarde, listar sus doce máquinas es ruido: están afuera con la clase de ahora y van a volver antes. Las liberadas se nombran siempre, porque son la excepción que alguien tiene que ver.
  - La devolución se marca en **una sola lista**, sin importar por qué salió la máquina: quien la recibe no tiene por qué acordarse de cómo se entregó. Va **agrupada por quién la tiene**: las computadoras salen con una clase y vuelven con esa clase, así que el caso normal es un botón y no uno por máquina. Recibir de a una sigue estando, como casilla adentro del grupo.
  - **La tira de números va arriba, antes del mostrador.** Es lo contrario de lo que decía esta línea, y el motivo cambió con ella: mientras los contadores eran cuentas por aprobar y avisos sin leer, el mostrador iba primero porque esto se opera con alguien esperando y aquello se mira una vez al día. Desde que la tira lleva *acá ahora* —con cuántas máquinas cuento si golpean la puerta— dejó de ser información administrativa y pasó a ser la primera pregunta del mostrador. Son cien píxeles que no crecen, y la cola sigue entrando en la primera pantalla.
  - **Lo que ya pasó hoy no es "lo que viene".** El backend responde "desde hoy" —no sabe qué hora es en la pantalla—, así que el corte por hora lo hace el cliente. Y para el Admin *Lo que viene* empieza **mañana**: lo de hoy ya está arriba, con más detalle, y repetirlo abajo llenaba la tarjeta con las mismas clases. El docente no tiene cola, así que para él sigue incluyendo hoy.
