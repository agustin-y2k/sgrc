...
Desde RF-01.3 el registro pregunta con qué **cargo** entra la persona, y una de las dos opciones se llama "Administrador de Sistema". **Eso no cambia lo anterior en nada**: el cargo declarado se guarda en `cargo_solicitado` y no participa de ninguna decisión de permisos. La cuenta nace `DOCENTE`/`PENDIENTE` igual, aprobarla sigue significando exactamente lo mismo, y para que administre el sistema hace falta el mismo `promover-a-admin` que para cualquier otra. Es un dato para que el Admin sepa a quién está mirando, no un pedido que el sistema conceda.
