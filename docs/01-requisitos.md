...
> Una escuela no presta solo las computadoras de sus carros: también proyectores, cargadores, notebooks de otro modelo. Parte de eso se puede planificar con anticipación —un proyector para una clase— y parte se pide en el momento. **Qué hay en esa lista cambia de institución en institución**, así que el sistema no puede traerla decidida.
...
>
> Durante un tiempo el costo fue que la tabla se llamaba `pc` y podía tener un proyector adentro. Se aceptó a propósito para no mezclar el renombre con el cambio de comportamiento, y se saldó en la migración 016: hoy la entidad se llama `equipo` en la base, en el código y en la API. `carro`, en cambio, se llama bien: es un mueble metálico con ruedas y zócalos numerados donde las notebooks se guardan y se cargan, y siempre está en el laboratorio de informática. **Cuántos zócalos tiene depende del carro** — el sistema no presupone ninguna cantidad: el identificador solo tiene que ser un entero positivo, sin techo. El `identificador` de un equipo es el número de su zócalo — de ahí que se repita entre carros y sea único dentro de uno.
...
- RF-05.11: Cuando un aviso va a **todos** los `ADMIN`, que falle uno no deja sin enterarse a los demás: se intenta con todos y los fallos se juntan para el registro. Cortar en el primero dejaba a los siguientes sin saber que hay una cuenta esperando aprobación o una máquina que no volvió. Mismo criterio que ya usaba el envío de correos.
...
- RF-08.3: **"¿Dónde está la PC 3?" se deriva, no se guarda.** No hay ninguna columna en `equipo` que diga "prestada": el estado sale de si existe una entrega sin devolver. Por eso no puede desincronizarse, que es exactamente lo que le pasa al papel cuando alguien devuelve una máquina y nadie tacha el renglón.
