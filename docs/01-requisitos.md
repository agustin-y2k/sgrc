...
>
> Durante un tiempo el costo fue que la tabla se llamaba `pc` y tenía un proyector adentro. Se aceptó a propósito para no mezclar el renombre con el cambio de comportamiento, y se saldó en la migración 016: hoy la entidad se llama `equipo` en la base, en el código y en la API. Queda `carro`, que es la otra imprecisión y merece su propia discusión: uno de los "carros" es un laboratorio fijo que no se mueve a ninguna parte.
...
- RF-04.10: Toda operación de lote —reservar, reservar en serie, bloquear para evaluación, entregar, recibir— tiene un **tope de 200 equipos**. No sale de ninguna regla de la escuela: el pedido lo arma el cliente, y sin tope mandar diez mil identificadores hace que el servidor intente diez mil filas en una transacción. Doscientos está muy por encima de cualquier uso real —el inventario son sesenta y pico y el pedido más grande es un carro entero— así que frena el pedido absurdo sin poder molestar a uno legítimo. La disponibilidad de todo el lote **se valida en una sola consulta**, no una por equipo: un bloqueo sobre un carro entero eran 64 idas a la base antes de escribir la primera fila.
