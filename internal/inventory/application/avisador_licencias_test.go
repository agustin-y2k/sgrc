...
func licenciaEnPC(t *testing.T, repo *fakeRepo, id, equipoID, nombre string, vencimiento *time.Time, diasAviso int) *domain.LicenciaSoftware {
	t.Helper()
...
func TestBarrer_LicenciaQueVenceManana(t *testing.T) {
	...
	licenciaEnPC(t, repo, "lic-1", "equipo-1", "AutoCAD 2027", &vence, 1)
	bus := &busEspia{}
...
func TestBarrer_LicenciaVencidaVaAlGrupoDeVencidas(t *testing.T) {
	...
	licenciaEnPC(t, repo, "lic-1", "equipo-1", "AutoCAD 2027", &vencio, 1)
	bus := &busEspia{}
...
func TestBarrer_OchoEquiposDelMismoCarroDanUnSoloAviso(t *testing.T) {
	...
	for i := 1; i <= 8; i++ {
		...
		licenciaEnPC(t, repo, "lic-"+string(rune('0'+i)), equipoID, "AutoCAD 2027", &vence, 1)
	}
	bus := &busEspia{}
...
func TestBarrer_SinNadaQueAvisarNoPublica(t *testing.T) {
	...
	licenciaEnPC(t, repo, "lic-1", "equipo-1", "AutoCAD 2027", &lejos, 1)
	// Y una sin fecha: cargada pero todavía sin verificar contra la máquina.
	licenciaEnPC(t, repo, "lic-2", "equipo-2", "SolidWorks", nil, 1)
	bus := &busEspia{}
...
func TestBarrer_UnaLicenciaSinFechaNoAvisaNunca(t *testing.T) {
	repo := repoConCarroYPCs(1)
	licenciaEnPC(t, repo, "lic-1", "equipo-1", "AutoCAD 2027", nil, 365)
	bus := &busEspia{}
...
func TestBarrer_EquipoDadaDeBajaNoAvisa(t *testing.T) {
	repo := repoConCarroYPCs(1)
	repo.equipos["equipo-1"].DadaDeBaja = true
	vence := dia(2026, time.August, 8)
	licenciaEnPC(t, repo, "lic-1", "equipo-1", "AutoCAD 2027", &vence, 1)
	bus := &busEspia{}
...
func TestBarrer_RenovarReabreElAviso(t *testing.T) {
	repo := repoConCarroYPCs(1)
	vence := dia(2026, time.August, 8)
	l := licenciaEnPC(t, repo, "lic-1", "equipo-1", "AutoCAD 2027", &vence, 1)
	bus := &busEspia{}
	ctx := context.Background()
