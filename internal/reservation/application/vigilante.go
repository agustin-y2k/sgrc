...
// ConfigDeVigilancia son los tres plazos que la escuela puede ajustar.
...
// corresponde. El resto son independientes.
...
// ── 3. Liberar lo que nadie retiró ──────────────────────────────────────

// liberarNoRetiradas es el corazón de la etapa: a los cuarenta minutos, una
...
func (v *Vigilante) liberarNoRetiradas(ctx context.Context, reservas []ReservaParaVigilar, ahora time.Time) int {
	...
	for _, grupo := range agruparPorGrupo(reservas) {
		...
		if !domain.CorrespondeLiberar(primera.Fecha, primera.HoraInicio, primera.HoraFin,
		...
		for _, r := range grupo {
			...
			equipos = append(equipos, r.Etiqueta)
			liberadas++
		}
		if len(equipos) == 0 {
			continue
		...
		aviso := eventbus.ReservasLiberadas{
			Email:           primera.DocenteEmail,
			Nombre:          primera.DocenteNombre,
			MateriaNombre:   nombreODefecto(primera.MateriaNombre),
			Fecha:           primera.Fecha,
			HoraInicio:      primera.HoraInicio,
			Equipos:         equipos,
			TodaLaReserva:   len(equipos) == len(grupo),
			MinutosDeGracia: int(v.cfg.GraciaDeRetiro.Minutes()),
		}
		if primera.DocenteID != nil {
			aviso.UsuarioID = *primera.DocenteID
		}
		v.bus.Publish(eventbus.Evento{Tipo: "reserva.no-retirada", Payload: aviso})
...
//
// Las que no tienen grupo —los bloqueos por evaluación estatal— quedan cada
...
// esDeUnDocente distingue la clase de alguien de un bloqueo por evaluación
// estatal (RF-04.7).
//
