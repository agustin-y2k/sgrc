...
// login/registro normal (internal/auth) nunca puedan divergir en cómo hashean
// o verifican.
package security
...
// VerifyPassword compara un password en texto plano contra un hash en formato
// PHC. El parseo es manual con strings.Split, no fmt.Sscanf — Sscanf con %s
// no delimita bien contra literales como '$' (solo contra espacios en
// blanco), así que hubiera partido mal la cadena.
func VerifyPassword(password, encoded string) (bool, error) {
	...
	// Sanity check: si estos vinieran en 0 (hash corrupto o editado a mano en la
	// base), argon2.IDKey podría comportarse de forma indefinida.
	...
	// El turno se toma acá y no al entrar: todo lo de arriba es parseo barato, y
	// ocupar un lugar de la cola mientras se valida el formato de un hash haría
	// esperar a un login legítimo por uno corrupto.
	defer tomarTurno()()
