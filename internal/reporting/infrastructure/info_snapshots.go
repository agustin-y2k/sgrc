...
var _ application.InfoEquipoParaSnapshot = (*InfoEquipoPostgres)(nil)

// InfoEquipoPostgres implementa el puerto hacia inventory — consulta pc/carro
// directamente, sin importar internal/inventory.
type InfoEquipoPostgres struct {
	pool *pgxpool.Pool
}

func NewInfoEquipoPostgres(pool *pgxpool.Pool) *InfoEquipoPostgres {
...
func (i *InfoEquipoPostgres) EtiquetaYCarroDe(ctx context.Context, equipoID string) (string, int, string, error) {
	...
	err := i.pool.QueryRow(ctx, `
		...
		WHERE p.id = $1
	`, equipoID).Scan(&etiqueta, &identificador, &carroNombre)
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return "", 0, "", fmt.Errorf("PC %s no encontrada", equipoID)
		}
		if esIDInvalido(err) {
